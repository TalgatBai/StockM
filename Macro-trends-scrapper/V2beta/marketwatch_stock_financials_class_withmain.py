# -*- coding: utf-8 -*-
"""
Created on Wed Oct 28 13:01:29 2020
...
class marketwatch_stock_financials_class:
    ...
    def fill_values(self, number_of_quarters):
        ...
        soup_results_array = self.soup.findAll("tr",{"class":"mainRow"})
        for soup_result in soup_results_array:
            for row in soup_result.findAll("td"):
                if (row.text.strip() == "EPS (Basic)") or (row.text == "EPS (Basic)"):
                    self.__q_eps_basic_array = self.get_growth_array(row, number_of_quarters)
                elif (row.text.strip() == "Basic Shares Outstanding"):
                    self.__q_basic_shares_outstanding_array = self.get_growth_array(row, number_of_quarters) 
        
        soup_results_array = self.soup.findAll("tr",{"class":"totalRow"})
        for soup_result in soup_results_array:
            for row in soup_result.findAll("td"):
                if (row.text.strip() == "Net Income") or (row.text == "Net Income"):
                    self.__q_net_income_array = self.get_growth_array(row, number_of_quarters)
        
        soup_results_array = self.soup.findAll("tr",{"class":"partialSum"})
        for soup_result in soup_results_array:
            for row in soup_result.findAll("td"):
                if (row.text.strip() == "Sales/Revenue") or (row.text == "Sales/Revenue"):
                    self.__q_sales_array = self.get_growth_array(row, number_of_quarters)
                elif (row.text.strip() == "Net Income Available to Common"):
                    self.__q_net_income_available_array = self.get_growth_array(row, number_of_quarters) 
    ...
    def get_q_eps_basic_array(self):
        return self.__q_eps_basic_array
    
    def get_q_basic_shares_outstanding_array(self):
        return self.__q_basic_shares_outstanding_array
    
    def get_q_net_income_array(self):
        return self.__q_net_income_array
    
    def get_q_sales_array(self):
        return self.__q_sales_array
    
    def get_q_net_income_available_to_common_array(self):
        return self.__q_net_income_available_array
